...
density_img = ax.imshow(np.zeros((10, 10)), origin='lower', extent=[-29, -11, 42, 47],
...
for x, y in processed_paths:
    ...
    start, = ax.plot(-25, 44.5, 'ko')  # Static black dot at origin
    dots.append(dot)
...
def update(frame):
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.plot(-25, 44.5, 'ro')  # Fixed red marker
